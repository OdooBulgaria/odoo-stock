...
class Picking(models.Model):
    ...
    def compute_invoice_total(self):
        ...
        for move in self.move_lines:
            if move.invoice_state == '2binvoiced' and move.state != 'cancel' and not move.scrapped:
                if move.procurement_id and move.procurement_id.sale_line_id:
                    sale_line = move.procurement_id.sale_line_id
                    if move.product_id.id != sale_line.product_id.id:
                        price_unit = self.pool['product.pricelist'].price_get(
                            cr, uid,
                            [sale_line.order_id.pricelist_id.id],
                            move.product_id.id, move.product_uom_qty or 1.0,
                            sale_line.order_id.partner_id,
                            context=self.env.context)[sale_line.order_id.pricelist_id.id]
                    else:
                        price_unit = sale_line.price_unit

                    price_unit = (price_unit * (1 - (sale_line.discount or 0.0) / 100.0))
                    ...
                    self.invoice_total += price_unit * move.product_uos_qty + sum(t.get('amount', 0.0) for t in taxes)
                else:
                    product = move.product_id.with_context(currency_id=move.company_id.currency_id.id)
                    price_unit = move.product_id.list_price and move.product_id.list_price or \
